...
async def command(cmd, topic, msg="", properties=None):
    await client.publish("control", json.dumps([cmd, topic, msg, properties]))
...
async def request_pub(topic, msg, properties=None):
    init([topic, msg, properties])
    await client.subscribe(topic, 1)
    await asyncio.sleep(1)
    # Request publication
    await command("publish", topic, msg, properties)
...
async def run_tests():
    ...
    r = await request_pub("foo topic", "hello", {3: "rats"})
